...
public class Fraccion {
     ...
     * @return
     */
    public static Fraccion getFraccionInversa(Fraccion frac) {
        Fraccion res = new Fraccion();
        res.num = frac.den;
        res.den = frac.num;
        return res;
     ...
     */
    public static Fraccion parseFraccion(String cadenaFraccion) throws FraccionFormatException {
        try
        {
        String arrayFrac[] = cadenaFraccion.split("/");
            Fraccion resultado;
            if (arrayFrac.length == 2) {
                if (!arrayFrac[1].equals("0")) {
                    resultado = new Fraccion(Long.parseLong(arrayFrac[0]), Long.parseLong(arrayFrac[1]));
                    return resultado;
                }else {
                    throw new FraccionFormatException("Denominador igual a Cero");
                }
            } else if (arrayFrac.length == 1) {
                resultado = new Fraccion(Long.parseLong(arrayFrac[0]), 1);
                return resultado;
            }else if(arrayFrac[0].equals("0")) {
                resultado = new Fraccion(0,0);
                return resultado;
            }else{
                throw new FraccionFormatException("Formato de fraccion incorrecto");
            }
        }
        catch(Exception e)
        {
     ...
     * @return
     */
    public static Fraccion parseFraccion(long fraccion) {
