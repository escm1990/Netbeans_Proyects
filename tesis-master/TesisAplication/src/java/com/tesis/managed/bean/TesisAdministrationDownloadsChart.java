 ...
 * @author luis.chavez
 */
@ManagedBean (name = "downloadChart")
@SessionScoped
public class TesisAdministrationDownloadsChart {

    /**
     * Creates a new instance of TesisAdministrationDownloadsChart
     */
    public TesisAdministrationDownloadsChart() {
    }
    ...
    public String calChartBar() {
        try {
            ...
            List<TesisAudit> listInfra = auditImp.findAllAuditByCriteria(from, to, "Infraestructura","descarga");
            chartInformation infra = new chartInformation("Infraestructura", String.valueOf(listInfra.size()));
            this.list.add(infra);
            List<TesisAudit> listOffer = auditImp.findAllAuditByCriteria(from, to, "Oferta","descarga");
            chartInformation oferta = new chartInformation("Oferta Académica", String.valueOf(listOffer.size()));
            this.list.add(oferta);
            List<TesisAudit> listCul = auditImp.findAllAuditByCriteria(from, to, "Cultura","descarga");
            chartInformation cultura = new chartInformation("Eventos Culturales", String.valueOf(listCul.size()));
            this.list.add(cultura);
            List<TesisAudit> listPlan = auditImp.findAllAuditByCriteria(from, to, "Planes","descarga");
            chartInformation plan = new chartInformation("Planes de Estudio", String.valueOf(listPlan.size()));
            this.list.add(plan);
            List<TesisAudit> listMa = auditImp.findAllAuditByCriteria(from, to, "Maestrias","descarga");
    ...
    private void createBarModel(List<TesisAudit> listin,List<TesisAudit> listOf,List<TesisAudit> listCu,List<TesisAudit> listPla,List<TesisAudit> listMa, Date from, Date to) {
        ...
        barModel.setTitle("Descargas realizadas al sistema del: "+ DatetoString.getDateD(from) + " Hasta: " + DatetoString.getDateD(to));
